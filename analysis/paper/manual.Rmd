...
The DABOM model requires as input a capture or detection history of every valid tag. This is a series of `1`'s and `0`'s depicting whether that tag was detected (or not) on each node in the network. The `DABOM` package includes a function to create these detection histories, based on the output from `PITcleanr`. To make the model easier to manipulate in the future, if more sites are added, we have split the overall detection histories into several matrices to feed into DABOM. The arguement `split_matrices = TRUE` in the function below does that. To create a single matrix with the entire detection history for each tag (for diagnostic purposes), simply set that arguement to `FALSE`. 
...
This version of DABOM was created to be run on both hatchery and wild fish, using all tags together to estimate detection probabilities, but allowing for different movement rates between hatchery and wild fish, since presumeably they may be going to very different places. Therefore, one of the inputs is a vector of fish origin (1 for wild fish, 2 for hatchery), called "fishOrigin". Currently, there are not enough hatchery tags to estimate separate movement probabilities by origin, so we are setting the origin to be equivalent for all tags, and adding it to input list `dabom_list`.

```{r}
dabom_list$fishOrigin = rep(1, nrow(dabom_list[[1]]))
```

Next, we write the text file that the JAGS software uses. There is a generic version for the Prosser dam DABOM:
...
We can then update that based on our tag detections. If no tags were detected at a particular node, we set that node's detection probability to 0, since it would be difficult if not impossible to estimate, or that node / site may not have existed for the year we are running the model. Similarly, if a terminal site contains only a single array, or only had detections on a single array, that array has its detection probability fixed to 100%, because without upstream detections there is no way to estimate the detection probability there. This may lead to conservative estimates of escapement past that particular site (i.e. escapement was *at least* this much), but since many terminal sites are further upstream in the watershed, where most detection probabilities are close to 100% already, this may not be a bad assumption to make. This function re-writes the generic JAGS model with one specific to the year we are running, and saves it as a different text file.
...
Based on the code in that model text file, we can extract the names of the parameters we wish to save. 
...
To actually run DABOM, we use the `jagsUI` package. Part of the inputs are the Monte Carlo Marchov chain (MCMC) parameters. We recommend:
...
This provides a total of 1000 draws from the posterior. In our experience, this leads to convergence of all the parameters, without taking too long to run. Setting the seed in `R` means you can reproduce the exact MCMC draws. 
...
What is returned (from the `jags.basic` function) is an `mcmc.list` object. The user can generate many diagnostic plots and statistics, using packages such as `coda`, `mcmcr` or even `shinystan`. 

## Summarise DABOM results

Detection probabilities are easily summarised by using the function `summariseDetectProbs`:
...
The movement or transition probabilities have to be extracted, and then multiplied appropriately, so that the movement past a site far upstream in network also accounts for moving past all the downstream sites before that one. There is a function specific to the Prosser Dam version of DABOM to do this. The "param" column in the `trans_summ` summary table refers to the probability of a tag moving *past* that site, or if the param ends in "_bb", the probability of falling into the black box above that site.
...
The next step is to multiply those transition probabilities by the total escapement at Prosser Dam. There is an additional R package, `STADEM` (*St*ate-space *A*dult *D*am *E*scapement *M*odel) that contains a function to query the window counts at a variety of dams within the Columbia River basin on a daily time-step. Those daily counts can be summed up for the season:
