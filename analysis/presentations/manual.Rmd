...
Set up a filter for specific tags by clicking on "27 Tag Code - List or Text File" on the right. Select "Import file..." and upload the file you saved in the previous step with all the tag codes in the valid tag list. Name the query something appropriate, such as "PTAGIS_2018_19", and select "Run Report". Once the query has been run, export the output as a csv file (e.g. "PTAGIS_2018_19.csv"). 

## Processing PTAGIS Detections

The next step is to clean up all the detections listed in the PTAGIS query. Those include every detection on every antenna, so we need to condense those to one detection on a particular array, even if the fish was detected 7 times on 3 different antennas in that array. We use the `PITcleanr` package for this. There are few parts to this particular step. The first is to load the appropriate packages into your R environment. 
...
The next step is to define which sites we are going to include in the DABOM model. The `PITcleanr` package has a special function to do this, called `writePRONodeNetwork`. This function lists all the various sites by their PTAGIS site ID code, and shows which other sites a tag would need to pass in order to reach that particular site.
...
The next step is to query PTAGIS for all the metadata associated with these sites. Again, `PITcleanr` has a function to do this, `buildConfig`, but you will need an internet connection to run this. This function returns information about each site in PTAGIS, including the site code, the various configuration codes, the antenna IDs, when that configuration started and ended (if it has), what type of site is it (interrogation, INT, or mark/recapture/recovery, MRR), the site name, the antenna group each antenna is part of, and several other pieces of information. It also assigns a model node to each antenna. These nodes are essentially which array each antenna is part of within each site. If it is a single array (or an MRR site), all the antennas will be assigned to the same node. If there is a double array, the antennas in the downstream array will be assigned to the "B" array, and the upstream antennas to the "A" array. If there is a triple array, by default the middle array is grouped with the upper array, to help simplify the DABOM model structure. This file is what will link the PTAGIS detections to the DABOM model nodes.

```{r}
org_config = buildConfig()
```

You then have the opportunity to modify this configuration file however you would like, re-assigning various antennas or sites to different nodes. For this version of DABOM, we have some suggestions, such as grouping all the various sites on the Teanaway River into a single node called "TEAN". Many of the modifications help simplify the nodes downstream of Prosser. 
